...
class _ClearancePageState extends State<ClearancePage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() =>
        Provider.of<PackageViewModel>(context, listen: false).loadClearancePackage());
  }
...
class UpdateClearancePackagePage extends StatefulWidget {
  const UpdateClearancePackagePage({super.key});

  @override
  State<UpdateClearancePackagePage> createState() => _UpdateClearancePackagePageState();
}

class _UpdateClearancePackagePageState extends State<UpdateClearancePackagePage> {
  ...
  void initState() {
    ...
    packageViewModel.loadClearancePackage();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Clearance Package", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
        centerTitle: true,
